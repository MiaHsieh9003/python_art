...
void art_trans_node4_to_node10(uint32_t old_track, uint16_t old_domain, uint32_t new_track, uint16_t new_domain) {
    ...
    if(old_domain == new_domain){
        ...
        align_shift = 8 * 8 + 1;
    }else { // domain difference is 1
        align_shift = 32 * 8 + 1;   
    }
    // printf("old_track: %d, old_domain:%d, new_track:%d, new_domain:%d\n", old_track, old_domain, new_track, new_domain);
    printf("vertical: %d, align: %d\n", vertical_shift, align_shift);
    art_shift(vertical_shift + align_shift);
    global_artskrm->cycle += 1;
}
...
int shift_count(int compare1_len, int compare2_len) {
    int max_shift = (compare1_len < compare2_len) ? compare2_len * 8 : compare1_len * 8;
    return (max_shift > global_artskrm->WORD_SIZE * 8) ? global_artskrm->WORD_SIZE * 8 : max_shift;
}
